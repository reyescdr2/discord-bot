...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('rule34')
    .setDescription('🔞 Motor dual (Rule34+Gelbooru) con filtro Malandro Bypass.')
    .addStringOption(option => 
      option.setName('busqueda')
        .setDescription('¿Qué buscamos? (Si no pones nada será random)')
        .setRequired(false)
    ...
    async autocomplete(interaction) {
        ...
        try {
            ...
            const resp = await axios.get(url + auth, { 
                timeout: 4000, // Aumentado para mayor resiliencia
                headers: { 
            ...
            if (choices.length > 0) {
                const uniqueChoices = choices
                    .filter((tag, index, self) => tag.name && self.findIndex(t => t.name === tag.name) === index)
                    .sort((a, b) => parseInt(b.count || 0) - parseInt(a.count || 0));

                await interaction.respond(
                    uniqueChoices.slice(0, 25).map(tag => ({ 
            ...
            logToFile(`[Autocomplete] Error en rule34: ${e.message}`, 'ERROR');
    ...
    async execute(interaction) {
        ...
        await nsfw.sendRule34(query, interaction, { isGif: false, isUpdate: interaction.isButton() });
    },
};
